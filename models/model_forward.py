...
def TransConv(in_feats,out_feats,kernel_size,stride,pad_size): # 4 by 4 stridee 2
...
class SRNet(nn.Module):
    def __init__(self):
        ...
        print('using forward scale')

    def forward(self,blur,n_level):
        inp_pred = blur
        inp_pred_list = []
        for i in range(n_level):
            scale_f = 0.5**(n_level-i-1)
            hi = int(round(256 * scale_f))
            wi = int(round(256 * scale_f))
            interpolation = nn.Upsample(size=[hi,wi], mode = 'bilinear',align_corners=True)
            inp_blur = interpolation(blur)
            inp_pred  = interpolation(inp_pred).detach()
            inp_all = torch.cat([inp_blur,inp_pred],dim=1)

            conv1_1 = self.enc1(inp_all)
            ...
            inp_pred = self.dec3(deconv3_3)
            inp_pred_list.append(inp_pred)

        return inp_pred_list
